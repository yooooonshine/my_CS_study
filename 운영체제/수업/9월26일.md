...
* context  switching이 어디서든 일어나도 상관없다. atomic하기에 
* down 사이는 크리티컬 리전
* empty,full은 이 변수를 동기화 하려고 쓰는 것이고, mutex는 buffer를 상호배제 하기 위해서 사용.
* ![[Pasted image 20231009172901.png]]
* 세마포어가 간단한 문제푸는 데는  좋지만 어려운 문제에는 적용하기 어렵다.
* 세마포어 조금만 줄 바꿔도 버그가 나므로 사용하기 어렵.(데드락)
* 
...
* item 만드는 것은 로컬이다. 즉 글로벌 하나도 안 쓴다.
* insert_item, remove_item은 모니터밖에 존재.
* 만약 if문 대신 while 문을 사용해도 된다. while 안에 wait 넣어주고, 꺠면 다시 while체크할 수 있도록 하면 좋다.
* 
* ![[Pasted image 20231009193939.png]]
* 호어는 시그널 보낸애가 블럭, 웨이팅한 애가 먼저
...
* 즉 느려질 수 있다.
* 모니터는 언어에서 제공, c,c++은 제공안해서 못 쓴다. 그냥 서버에서 구현하면 된다. 그냥 없다는 것.
* 만약 언어가 제공 안하면 못 쓴다.
